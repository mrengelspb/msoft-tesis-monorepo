...
class Graph:
    ...
    def update(self):
        ...
        for count, channel in enumerate(self.ecg_channels):
            ...
            if count == 0:
                try:
                    # --- CORRECCIÓN: Copiamos el array filtrado ---
                    # Creamos una copia nueva y limpia del array que YA FUE FILTRADO
                    ecg_data_para_bpm = np.copy(ecg_data)
                    # --- FIN DE LA CORRECCIÓN ---

                    # --- DEBUG: Imprimimos los argumentos ---
                    arg1 = ecg_data_para_bpm # <--- Pasamos la copia limpia
                    arg2 = self.sampling_rate
                    arg3 = self.hr_psd_size
                    arg4 = self.hr_psd_size // 2
                    
                    print(f"--- Debug BPM Inputs ---")
                    print(f"  arg1 (ecg_data) type: {type(arg1)}")
                    print(f"  arg1 (ecg_data) shape: {arg1.shape}")
                    print(f"  arg2 (sampling_rate): {arg2}")
                    print(f"  arg3 (hr_psd_size): {arg3}")
                    print(f"  arg4 (overlap): {arg4}")
                    # --- Fin Debug ---

                    self.bpm = DataFilter.get_heart_rate(arg1, arg2, arg3, arg4)
            ...
            # 4. Graficado
            # Usamos el array 'ecg_data' original (el que fue filtrado y está "dañado")
            # para el graficador. A pyqtgraph no le importa su estado interno.
